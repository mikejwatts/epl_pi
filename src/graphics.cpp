...
{
	if (pdc) {
		// PPW Testing rollover bug
		//wxMessageBox(wxT("Called render poly!"));
...
#ifdef ocpnUSE_GL
		// PPW Testing rollover bug
		//wxMessageBox(wxT("Called render poly want an opengl!"));
		/* opengl version */
		glEnable(GL_BLEND);
		glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
		glColor4ub(color.Red(), color.Green(), color.Blue(), transparency);
...
/*----------------------------------------------------------------------
* Render a N-pointed 'star' or tripod at the location and diameter
* provided along with the colour and transparency given.  Angle sets
* the position 0 = [first arm] pointing N, PI / 2 = pointing E.
*
* @param pdc [in]			pointer to the graphics display controller -
*							NULL for OpenGL engine
* @param x [in]			centre x position in pixels
* @param y [in]			centre y position in pixels
* @param n [in]			number of points
* @param diameter [in]		diameter of envelope in pixels
* @param angle [in]		attitude of the points in radians
* @param color [in]		fill colour
* @param transparency [in] transparency, 255 = solid
*--------------------------------------------------------------------*/
void RenderNpoint(wxDC *pdc, int x, int y, int n, int diameter,
	...
	float arm_arg = 2 * PI / n;			// angular difference between points
	...
	if (pdc) {
		...
		for (int i = 0; i < n; i++) {
			float arg = angle + i * arm_arg;
			pdc->DrawLine(x, y, x + r * sin(arg), y - r * cos(arg));
		}
		...
		for (int i = 0; i < n; i++) {
			float arg = angle + i * arm_arg;
			glVertex2i(x, y);
			glVertex2f(x + r * sin(arg), y - r * cos(arg));
		}
		glEnd();
